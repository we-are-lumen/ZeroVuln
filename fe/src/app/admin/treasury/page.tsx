...
import { useFundZvTreasury, useQueryZvTreasuryBalance } from "./hooks/use-zv-treasury";

const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZV_CONTRACT_ADDRESS;

export default function AdminTreasuryPage() {
  const { data: balanceWei, isLoading, isError, error } = useQueryZvTreasuryBalance();
      ...
      <div className="flex items-center justify-between border-b pb-3">
        <div>
          ...
          <p className="text-sm text-mist-500">
            Top up saldo ZVContract untuk bayar reward claimable.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card className="border bg-mist-900/50 backdrop-blur-sm">
          <CardHeader className="border-b">
            ...
            <CardDescription>Info dasar ZVContract yang dipakai FE.</CardDescription>
          </CardHeader>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {isLoading ? (
                ...
                <span className="text-xs text-destructive">
                  {error instanceof Error ? error.message : "Gagal load balance."}
                </span>
              ) : (
                <span className="text-sm font-bold tabular-nums">{balance0g} 0g</span>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border bg-mist-900/50 backdrop-blur-sm">
          <CardHeader className="border-b">
            ...
            <CardDescription>Kirim 0G dari wallet admin ke contract.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
