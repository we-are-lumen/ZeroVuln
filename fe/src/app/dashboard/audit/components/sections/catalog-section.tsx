...
const CatalogSection = () => {
  ...
  return (
    <section className="h-full basis-[25%] rounded-lg border">
      <div className="border-b px-6 py-3">
        <h3 className="text-sm font-bold text-mist-400">CATALOG</h3>
      </div>
      <div className="grow space-y-3 overflow-y-auto p-6 pt-3">
        {data?.map(({ uuid, name, reward_per_finding }) => (
          <div key={uuid} className="rounded-md border p-3">
            <h4 className="font-semibold">{name}</h4>
            <div className="mt-5 flex items-end justify-between">
              <p className="text-xs text-muted-foreground">
                {reward_per_finding}
              </p>
              <Button size="sm" variant="outline" onClick={handleSelect(uuid)}>
                See Detail
              </Button>
            </div>
          </div>
